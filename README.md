...
# 16  纯函数 纯虚函数
纯函数 在虚幻中没有执行引脚，必须有返回值
纯虚函数 是在 抽象类中, C++ 中包含纯虚函数的类叫做抽象类吗抽象类不能使用 new 来实例化对象，只有实现了纯虚 函数的子类才能 new 实例化对象； C++ 中纯虚函数 只提供声明没有实现， 对子类的约束，是 接口继承； C++ 中的纯虚函数也还是运行多态



重载	重写
重载 ：是在同一个类中，函数名相同，参数不同，返回值可以相同也可以不同
重写 ： 是 子类 重写 父类 的 虚函数， 重写 的 函数 必须和 父类 的 函数 完全相同， 返回值，函数名，参数列表
...
# 49.虚幻C++共享引用
共享引用： 共享指针的增强版， 共享指针 不能赋值为空， 共享引用可以赋值为空


# 50.虚幻C++弱指针
弱指针 是为了解决 循环引用的问题， 循环引用 会导致内存泄漏
